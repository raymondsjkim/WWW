...
set_time_limit(720);
	...
	while($row = mysql_fetch_assoc($result)){
		...
		//Lingo EDI takes a really werid format and conflicts with Nordstrom - in order to 
		//take an item off Nordstrom dropship, we need to input a zero-stock line item at
		//least once. However, Lingo doesn't allow zero-stock items to be input into EDI.
		// The only way around it is to mark it as discontinuted momentarily
		$zeroStock = ($tmpStock < 1 ? "12/31/2039" : "");

		if ($tmpSeason != 'CLOSEOUT') {
			$line = "NORDSTROM_DI,".$tmpUPC.",,".$tmpSize.",,,EA,".$tmpColor.",,,,".$tmpStock.",,,".$zeroStock.",".$tmpWholeSale.",,,Y,N,,,,,,,,,,,\r\n";
	...
	fclose($file);
	echo 'Nordstrom Lingo CSV Created Successfully! Date today: '.$date.'<br /> <br />';
	echo 'Download: <a href="ftp://lko@ftp.alegriashoes.com/www/sales_alegriashoes_com/nordstrom/nordstrom_inventory'.$date.'.csv">Nordstrom LINGO CSV for '.$date.'</a><br /><br /><b>***RIGHT CLICK and SAVE-AS to save file to computer!</b>';